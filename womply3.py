...
#Program to calculate the churn rate for the ee group who joined on January, 2014.
year = 2014
...
def calculate_monthly_churn(clientIDs, cnx):
    ...
    while True:
        ...
        try:
            ...
            dateKey =  str(month) + '/' + str(year)[2:4]
                ...
                if cursor:
                    ...
                    if clientswhowereactivex:
                        ...
                        if clientswhowereactive > 0:
                            ...
                            churndict[dateKey ] =  float(churnd)
                        else:
                            churndict[dateKey ] = 0
                    else:
                        churndict[dateKey ] = 0
...
def get_cohorts(cnx):
    billcohort = []
    query = ("SELECT id ID, min(BillDate) SubDate FROM billing.billing_table group by id having min(BillDate) < '2014-02-01' and min(BillDate) > '2013-12-31' ")
    cursor = cnx.cursor()
...
def create_chart(get_array):
    ...
    rects1 = pl.bar(index, xaxis, bar_width,alpha=opacity, color='b', label='Jan-14 Churn Rate')

    pl.xlabel('Date')
    pl.ylabel('Churn Rate')
    pl.title('Monthly Churn Rate of Jan14 Cohort')
